...
public class CargoDAO implements CargoINT{
    ...
    @Override
    public Boolean agregarRegistro(CargoDTO dto) {
       try {
            ...
            query = "INSERT INTO public.cargos(car_descri) VALUES ( ? );";
    ...
    @Override
    public Boolean modificarRegistro(CargoDTO dto) {
    try {
            //// PASO 1 - PREPARAR TRANSACCION 
            conexion.Transaccion(ConexionBD.TR.INICIAR);
            // PASO 2 - PREPARAR EL QUERY SQL
            ...
            // 1. Verificar la correcta escritura de la sintaxis ( nombre de tabla, nombres de columnas)
            // 2. Verificar la cantidad de columnas vs los simbolos que representan a los valores parametricos ( ? ) . "Misma cantidad de columnas es igual a misma cantidad de parametros"
            query = "UPDATE public.cargos SET car_descri = ? "
                    + " WHERE car_codigo = ?;";
            // PASO 3 - UTILIZAR EL OBJETO CONEXION PARA PREPARAR EL STATEMENT
            //REGLA 
            // ENCAPSULAR LAS SINTAXIS EN UN BLOQUE TRY-CATCH
    ...
    public Boolean eliminarRegistro(Integer id) {
      try {
            ...
            // 1. Verificar la correcta escritura de la sintaxis ( nombre de tabla, nombres de columnas)
            // 2. Verificar la cantidad de columnas vs los simbolos que representan a los valores parametricos ( ? ) . "Misma cantidad de columnas es igual a misma cantidad de parametros"
            query = "DELETE FROM  public.cargos WHERE car_codigo = ?;";
            // PASO 3 - UTILIZAR EL OBJETO CONEXION PARA PREPARAR EL STATEMENT
            //REGLA 
            // ENCAPSULAR LAS SINTAXIS EN UN BLOQUE TRY-CATCH
            ps = conexion.obtenerConexion().prepareStatement(query);
    ...
    public List<CargoDTO> consultarTodos() {
      try {
            ...
            //REGLAS
            // 1. Verificar la correcta escritura de la sintaxis ( nombre de tabla, nombres de columnas)
            query = "SELECT car_codigo, car_descri FROM public.cargos;";
            ...
            while (rs.next()) {
                // EXITOSA
                dto = new CargoDTO();
                dto.setId(rs.getInt("car_codigo"));
                dto.setDescrip(rs.getString("car_descri"));
    ...
    @Override
    public CargoDTO consultarSegunID(Integer id) {
      try {
            CargoDTO dto = null;
            // PASO 1 - PREPARAR EL QUERY SQL
            //REGLAS
            // 1. Verificar la correcta escritura de la sintaxis ( nombre de tabla, nombres de columnas)
            query = "SELECT car_codigo, car_descri FROM public.cargos WHERE car_codigo = ? ;";
            // PASO 2 - UTILIZAR EL OBJETO CONEXION PARA PREPARAR EL STATEMENT
            //REGLA 
            // ENCAPSULAR LAS SINTAXIS EN UN BLOQUE TRY-CATCH
            ps = conexion.obtenerConexion().prepareStatement(query);
